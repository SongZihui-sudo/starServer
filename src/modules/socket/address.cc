...
static star::Logger::ptr g_logger( STAR_NAME( "g_logger" ) );
                                     ...
                                     int family )
{
    ...
    {
        DEBUG_STD_STREAM_LOG( g_logger ) << "Address::GetInterfaceAddresses exception%n%0";
        freeifaddrs( results );
        return false;
    }
    freeifaddrs( results );
...
void IPv4Address::setPort( uint16_t v ) { m_addr.sin_port = byteswapOnBigEndian( v ); }
...
void IPv6Address::setPort( uint16_t v ) { m_addr.sin6_port = byteswapOnBigEndian( v ); }
